...
const MySwal = withReactContent(Swal)
 ...
 const handleDelete = (id) => {
  ...
  const handleSearch = (searchData,users)=>{
      ...
      const result = users.filter((doc)=>regex.test(doc.fname))
      return result
  }
  ...
  return (
    ...
    <Toolbar style={{ justifyContent: "space-between" }}>
      { state ? (
        ...
        <Link to={'/admin/login'}> <Button variant="contained" color="success" nowrap>
          ...
          <tbody>
           {
            users?.map((user,index)=>{
                ...
                <tr key={index}>
                    ...
                    <th><Link to={user?.photo}><img src={user?.photo}style={{width:'80px',height:'80px'}} /></Link></th>
                    <td>{user.fname}</td>
                    <td>{user.lname}</td>
                    <td>{user.email}</td>
                    <td>{user.phone}</td>
